        ...
        if (channelName[0] == '#') {
            ...
            if (joinedChannel != NULL)
            {
                ...
                if (userJoined == 0)
                {
                    ...
                    std::cout << "[CHANNEL] '~" << _clientList[i]->_nickname << "' has joined channel '" << channelName << "'." << std::endl;
                    _clientList[i]->_joinedChannel = channelName;
                    /*
                        user modes
                    */
            ...
            // In case the channel doesn't exists before, create a new one
                ...
                std::cout << "[CHANNEL] A new chat space " << channelName << " has been created by ~" << _clientList[i]->_nickname << "."<< std::endl;
